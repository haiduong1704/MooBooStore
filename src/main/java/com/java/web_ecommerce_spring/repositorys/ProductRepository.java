...
public interface ProductRepository extends PagingAndSortingRepository<Product,Integer> {
    ...
    @Query(value = "SELECT product_id FROM order_detail WHERE order_id IN (SELECT id FROM orders WHERE status = 3) GROUP BY product_id ORDER BY SUM(discount) DESC ",nativeQuery = true)
    List<Integer> listID();

    @Query(value = "SELECT * FROM product WHERE id NOT IN (SELECT product_id FROM order_detail) ",nativeQuery = true)
